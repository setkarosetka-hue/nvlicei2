...
const PUBLIC_PAGES = ['', 'index.html', 'consent.html', 'login.html', 'maintenance.html', 'attendance.html'];
const PAGE_ROLES = {
  'admin.html': ['admin'],
...
function hasValidConsent() {
  ...
  return !!(consent && consent.version === SYSTEM_CONFIG.rulesVersion && consent.accepted === true);
}
...
function isMaintenanceEnabled() {
  ...
  return !!(maintenance && maintenance.enabled === true);
}
...
async function checkRoleAccess() {
  ...
  if (!allowedRoles.includes(profile.role)) {
    ...
    window.location.href = rolePages[profile.role] || 'dashboard.html';
    return false;
  }
...
function toggleMenu() {
  document.querySelectorAll('.navigation, .main-nav').forEach((nav) => {
    nav.classList.toggle('mobile-open');
  });
}

function setupMobileMenu() {
  document.querySelectorAll('.mobile-menu, .mobile-menu-btn').forEach((button) => {
    button.addEventListener('click', toggleMenu);
  });
}

function setupScrollAnimations() {
  const elements = document.querySelectorAll('.card, .feature-card, .stat-card, .section');
...
async function updateUserElements() {
  ...
  document.querySelectorAll('[data-user-name]').forEach((element) => {
    element.textContent = profile?.full_name || profile?.email || 'Користувач';
  });
  document.querySelectorAll('[data-user-email]').forEach((element) => {
    element.textContent = profile?.email || '';
  });
  document.querySelectorAll('[data-user-role]').forEach((element) => {
    element.textContent = getRoleName(profile?.role);
  });
}

function setupLogoutButtons() {
  document.querySelectorAll('[data-logout], .logout-button, #logoutButton, #logoutBtn').forEach((button) => {
...
function setupConsentButtons() {
  const allButton = document.getElementById('acceptAllRules');
  const requiredButton = document.getElementById('acceptRequiredRules');
  const customButton = document.getElementById('acceptCustomRules');
...
function updateSystemStatus() {
  document.querySelectorAll('[data-system-status]').forEach((element) => {
    element.textContent = SYSTEM_CONFIG.officiallyApproved ? 'Офіційно підтверджено' : 'У розробці';
  });
}
...
async function initializeApp() {
  ...
  const roleAccess = await checkRoleAccess();
